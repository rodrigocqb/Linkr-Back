...
const getSharedPosts = async (followerId) => {
    ...
    SELECT shares.post_id as id, shares.id as shared_id, u2.id as user_id, u2.username, posts.link, posts.description, shares.created_at, u1.id as shared_by_id, u1.username as shared_by_username,
     ARRAY_REMOVE(
    ...
    GROUP BY u1.id, shares.post_id, shares.id, u2.id, posts.link, posts.description
